...
const Representantes = () => {
  ...
  const teamMembers = [
    ...
    {
      name: "Luis Mendoza",
      position: "Ingeniero Aeroespacial",
      imageUrl: "https://picsum.photos/id/401/300/300",
    },
    {
      name: "Elena Ramos",
      position: "Diseñadora de Sistemas",
      imageUrl: "https://picsum.photos/id/402/300/300",
    },
    {
      name: "Jorge Soto",
      position: "Especialista en Propulsión",
      imageUrl: "https://picsum.photos/id/403/300/300",
    },
  ];
  ...
  useEffect(() => {
    const profileImages =
      profileImagesContainerRef.current.querySelectorAll(".img");

    // Mapea las referencias a los elementos `h1` y `h2` reales
    const memberNameHeadings = nameElementsRef.current
      .slice(1)
      .map((el) => el && el.querySelector("h1"))
      .filter(Boolean);
    ...
    if (defaultNameHeadingRef.current) {
      const split = new SplitType(defaultNameHeadingRef.current, {
        types: "chars",
      });
    ...
    const defaultLetters = defaultNameHeadingRef.current
      ? defaultNameHeadingRef.current.querySelectorAll(".char")
      : [];
    // Crea arrays de arrays de letras (cada sub-array son las letras de un nombre/cargo)
    const allMemberNamesLetters = memberNameHeadings.map((heading) =>
      heading.querySelectorAll(".char"),
    );
    const allMemberPositionsLetters = memberPositionHeadings.map((heading) =>
      heading.querySelectorAll(".char"),
    );

    // Inicializa el estado (oculta nombres y cargos de miembros, muestra el default)
    gsap.set(defaultLetters, { y: "0%" });
    allMemberNamesLetters.forEach((letters) =>
      gsap.set(letters, { y: "100%" }),
    );
    allMemberPositionsLetters.forEach((letters) =>
      gsap.set(letters, { y: "100%" }),
    );
    ...
    if (window.innerWidth >= 900) {
      ...
      return () => {
        if (profileImagesContainerRef.current) {
          profileImagesContainerRef.current.removeEventListener(
            "mouseleave",
            handleContainerMouseLeave,
          );
        }
  ...
  return (
    <div className="page4 min-h-screen flex flex-col items-center">
      ...
      <div
        ...
        className="scroll-indicator w-full bg-white flex flex-col justify-center items-center text-black"
      >
      ...
      <section className="team w-full min-h-screen flex flex-col justify-center items-center bg-white text-black">
        ...
        <div className="profile-names">
          ...
          {teamMembers.map((member, index) => (
            ...
            >
              <h1>{member.name}</h1>
              <h2 ref={(el) => (positionElementsRef.current[index] = el)}>
                {member.position}
              </h2>
            </div>
          ))}
