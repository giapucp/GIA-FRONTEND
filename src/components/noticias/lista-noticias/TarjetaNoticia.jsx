...
const TarjetaNoticia = ({ noticia, onClick }) => {
  const titulo = noticia.titulo || noticia.attributes?.titulo || 'Título no disponible';
  const imagen = noticia.portada || noticia.attributes?.portada || '/default-image.jpg';
  const fecha = noticia.fecha || noticia.attributes?.fecha || new Date().toISOString();
